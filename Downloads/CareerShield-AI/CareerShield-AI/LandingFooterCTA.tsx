import React from 'react';
import Link from 'next/link';
import { Shield, Radar } from 'lucide-react';
import AppLogo from '@/components/ui/AppLogo';

export default function LandingFooterCTA() {
  return (
    <footer className="border-t border-slate-800/60">
      {/* CTA section */}
      <div className="px-6 py-20 max-w-screen-2xl mx-auto lg:px-8 xl:px-10 2xl:px-16 text-center">
        <div className="max-w-2xl mx-auto">
          <div className="w-16 h-16 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mx-auto mb-6">
            <Shield size={28} className="text-red-400" />
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold text-slate-100 mb-4">
            Scan Before You Apply
          </h2>
          <p className="text-slate-500 mb-8 leading-relaxed">
            Every job posting you scan is one less opportunity for a fraudster to steal your personal data, time, or money. Free, instant, and private.
          </p>
          <Link
            href="/scanner-dashboard"
            className="btn-primary-glow inline-flex items-center gap-2.5 px-10 py-4 rounded-xl text-base font-semibold text-white"
          >
            <Radar size={18} />
            Launch Free Scanner
          </Link>
          <p className="mt-4 text-xs font-mono text-slate-600">
            No account required · No data stored · No credit card
          </p>
        </div>
      </div>

      {/* Footer bar */}
      <div className="border-t border-slate-800/60 px-6 py-6 max-w-screen-2xl mx-auto lg:px-8 xl:px-10 2xl:px-16">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <AppLogo size={24} />
            <span className="text-sm font-medium text-slate-500">
              CareerShield<span className="text-red-500">AI</span>
            </span>
          </div>
          <p className="text-xs font-mono text-slate-600 text-center">
            © 2026 CareerShieldAI · Built to protect job seekers globally
          </p>
          <div className="flex items-center gap-3">
            <a href="#" className="text-slate-600 hover:text-slate-400 transition-colors" aria-label="GitHub">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/></svg>
            </a>
            <a href="#" className="text-slate-600 hover:text-slate-400 transition-colors" aria-label="Twitter">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}