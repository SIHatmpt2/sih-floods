...
class RiskServiceContractTests(SimpleTestCase):
    @patch("backend.services.risk_service.nearest_zone", return_value=None)
    @patch("backend.services.risk_service.assess_point")
    @patch("backend.services.risk_service.cache.get_current", return_value=None)
    @patch("backend.services.risk_service.cache.set_current")
