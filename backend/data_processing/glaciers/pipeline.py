"""Glacier/lake CSV -> Parquet pipeline."""
...
from .clean import clean_glacier_data
from .features import add_glacier_features

LOGGER = logging.getLogger(__name__)
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_DATA_ROOT = PROJECT_ROOT / "apps" / "risk" / "data"
DEFAULT_RAW_DIR = DEFAULT_DATA_ROOT / "raw" / "glaciers_data"
DEFAULT_OUTPUT_DIR = DEFAULT_DATA_ROOT / "processed" / "glaciers_data" / "parquet"
...
def discover_csv_files(raw_dir: Path) -> list[Path]:
    ...
    if not raw_dir.exists():
        raise FileNotFoundError(f"Raw glacier-data directory does not exist: {raw_dir}")
...
def _output_name(input_path: Path) -> str:
    return f"{input_path.stem.lower().replace(' ', '_').replace('-', '_')}.parquet"


def process_file(input_path: Path, output_dir: Path) -> Path:
    """Read, clean, feature-engineer and write one CSV as compressed Parquet."""
    input_path = Path(input_path)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    frame = pd.read_csv(input_path, low_memory=False)
    cleaned = clean_glacier_data(frame, input_path)
...
def run_pipeline(raw_dir: Path | None = None, output_dir: Path | None = None, combine: bool = True) -> dict[str, object]:
    """Process all glacier/lake CSVs and return generated output paths."""
    ...
    combined = _write_combined(per_file, output_dir / "glacier_changes.parquet") if combine else None
    return {"per_file": per_file, "combined": combined}


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s: %(message)s")
    LOGGER.info("Glacier pipeline outputs: %s", run_pipeline())
