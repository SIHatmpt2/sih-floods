...
class SegmentationDatasetTests(unittest.TestCase):
    ...
    def test_prepare_dataset_pairs_normalizes_masks_and_splits_without_duplicate_leakage(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self._write_mask(masks / "scene-0-duplicate_mask.png", [mask_values] * 8)
            ...
            self.assertEqual(summary["empty_masks"], [])
    ...
    def test_prepare_dataset_rejects_identical_images_with_conflicting_masks(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            images = root / "images"
            masks = root / "masks"
            ...
            self._write_image(images / "scene.jpg", 40)
            (images / "scene-duplicate.jpg").write_bytes((images / "scene.jpg").read_bytes())
            self._write_mask(masks / "scene_mask.png", [[255] * 10] * 8)
            self._write_mask(masks / "scene-duplicate_mask.png", [[0] * 10] * 8)

            with self.assertRaisesRegex(DatasetValidationError, "conflicting masks"):
                prepare_dataset(images, masks, output)

    def test_prepare_dataset_rejects_empty_masks_by_default(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            images = root / "images"
            masks = root / "masks"
            ...
            self._write_image(images / "scene.jpg", 40)
            self._write_mask(masks / "scene_mask.png", [[0] * 10] * 8)

            with self.assertRaisesRegex(DatasetValidationError, "empty masks"):
                prepare_dataset(images, masks, output)

    def test_prepare_dataset_can_explicitly_allow_empty_masks_and_reports_them(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            images = root / "images"
            masks = root / "masks"
            ...
            summary = prepare_dataset(images, masks, output, allow_empty_masks=True)
            self.assertEqual(summary["samples"], 1)
            self.assertEqual(summary["empty_masks"], ["scene_mask.png"])
    ...
    def test_prepare_dataset_fails_when_a_unique_image_has_no_mask(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self._write_mask(masks / "covered_mask.png", [[255] * 10] * 8)
