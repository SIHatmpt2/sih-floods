"""Historical flood-event CSV -> Parquet pipeline."""
...
from .clean import clean_past_events_data
from .features import add_event_features
...
def process_file(input_path: Path, output_dir: Path) -> Path:
    ...
    cleaned = clean_past_events_data(frame, input_path)
...
def run_pipeline(raw_dir: Path | None = None, output_dir: Path | None = None, combine: bool = True) -> dict[str, object]:
    ...
    combined = _write_combined(per_file, output_dir / "past_flood_events.parquet") if combine else None
    return {"per_file": per_file, "combined": combined}


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s: %(message)s")
    LOGGER.info("Past-event pipeline outputs: %s", run_pipeline())
